...
public interface EnseignantRepository extends JpaRepository<Enseignant, Long> {
    ...
    // 🆕 Pour le multi-instituts - Version Pageable
    Page<Enseignant> findByInstitutId(Long institutId, Pageable pageable);

    // 🆕 Version List pour les selects
    List<Enseignant> findByInstitutId(Long institutId);

    // 🆕 Enseignants actifs par institut
    @Query("SELECT e FROM Enseignant e WHERE e.institut.id = :institutId AND e.active = true")
    Page<Enseignant> findActifsByInstitutId(@Param("institutId") Long institutId, Pageable pageable);
}