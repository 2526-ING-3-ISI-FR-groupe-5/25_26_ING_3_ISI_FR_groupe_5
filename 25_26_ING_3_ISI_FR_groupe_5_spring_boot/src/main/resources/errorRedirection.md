...
```bash
mvn spring-boot:run 2>> error.txt
```
...
```bash
mvn spring-boot:run 2>&1 | Select-String -Pattern "ERROR|WARN|Caused by|Exception" > error.txt
```

## Solution 3 : Filtrer avec `findstr` (CMD Windows)

```bash
mvn spring-boot:run 2>&1 | findstr /I "ERROR WARN Caused Exception Failed" > error.txt
```

## Solution 4 : Script bash complet (Linux/Mac/Git Bash)

```bash
mvn spring-boot:run 2>&1 | grep -E "ERROR|WARN|Caused by|Exception|Failed|BUILD" > error.txt
```

## Solution 5 : N'afficher que les erreurs Spring Boot

```bash
mvn spring-boot:run -q 2>&1 | grep -A 5 "ERROR" > error.txt
```
...
```bash
mvn spring-boot:run -Dlogging.level.root=ERROR 2>&1 | grep -v "INFO" > error.txt
```
